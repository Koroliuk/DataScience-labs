...
G_group_normalized = np.zeros(len(G_group))
G_sum = sum(G)
G_group_sum = sum(G_group)
for i in range(factors_count):
    G_normalized[i] = G[i] / G_sum

G_group_sum = sum(G_group)
for i in range(len(G_group)):
    G_group_normalized[i] = G_group[i] / G_group_sum
...
for i in range(elements_count):
    for j in range(4):
        integro[i] += G_group_normalized[j] * (1 - factors_by_group_normalized[j, i]) ** (-1)
